...
- **多行事曆勾選**：長按月曆區可挑選要顯示哪些行事曆（一個 Google 帳號常有多個，如節日曆／家庭／個人），對話框內也有「管理 Google 帳號」捷徑直達系統帳號設定（新增/重新登入/手動同步都在那邊，App 本身不做 OAuth）
...
- **自動更新**：每小時整頁刷新一次，並在偵測到真實日期跨天時自動跳回新的「今天」；每次 App 從背景回到前景也會重新查詢一次資料
